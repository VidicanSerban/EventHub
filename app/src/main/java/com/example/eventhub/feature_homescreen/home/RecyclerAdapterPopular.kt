...
class RecyclerAdapterPopular(var list: List<Event>): RecyclerView.Adapter<RecyclerAdapterPopular.ViewHolder>() {
...
//    private var eventlocation = arrayOf("Oradea, Romania", "Oradea, Romania", "Oradea, Romania", "Oradea, Romania")
    ...
    override fun onBindViewHolder(holder: RecyclerAdapterPopular.ViewHolder, position: Int) {
        ...
        holder.itemDate.text = list[position].date
        holder.itemTime.text = list[position].time
        holder.itemLocation.text = list[position].location
    }
